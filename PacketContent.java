...
public abstract class PacketContent {
	...
	public static final int CLIENTNAME= 100;
	int type = 0;
	...
	public static PacketContent fromDatagramPacket(DatagramPacket packet) {
		...
		try {
			...
			case RESULTPACKET:
				content= new ResultPacket(oin);
				break;
			case WORKERPACKET:
				content= new WorkerPacket(oin); //for shane to make!
				break;
			case CLIENTNAME:
				content= new SendName(oin);
				break;
			case HEARTBEAT:
				content= new Heartbeat(oin); //for david to make!
				break;
			case REGISTER:
				content= new Register(oin);
				break;
			case STOPWORK:
				content= new StopWork(oin);
				break;
			default:
				content= null;
				break;
